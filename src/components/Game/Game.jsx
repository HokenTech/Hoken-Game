...
class Game extends Component {
  ...
  loadUser() {
    ...
    if (this.context == undefined || this.context.activeUser == null) {
      //console.log(this.context)
      //console.log('calling setUser from')
      setUser({
        registered: false,
        name: "",
      })
      ...
      return ApiService.getUserByName(name).then(user => {
        //console.log("calling setUser from game")
        setUser({
          registered: true,
  ...
  handleStartGame() {
    ...
    return ApiService.startGame(this.context).then(() => {
  ...
  handlePlayCard(cardIdx) {
    ...
    // And call `loadUser` again for react to render latest game status to UI
    return ApiService.playCard(cardIdx, this.context).then(() => {
      return this.loadUser();
    });
  }
  ...
  handleNextRound() {
    ...
    // And call `loadUser` again for react to render latest game status to UI
    return ApiService.nextRound(this.context).then(() => {
      return this.loadUser();
    });
  }
  ...
  handleEndGame() {
    ...
    // And call `loadUser` again for react to render latest game status to UI
    return ApiService.endGame(this.context).then(() => {
      return this.loadUser();
    });
  }
